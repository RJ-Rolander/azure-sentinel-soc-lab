...
| Path | What it holds |
| --- | --- |
| [`lab-setup.md`](lab-setup.md) | Build detail: Arc onboarding, Azure Monitor Agent, DCR and XPath subscriptions, audit policy |
| [`docs/detections.md`](docs/detections.md) | The four analytics rules: logic, scheduling, entity mapping, ATT&CK IDs, false positive sources |
| [`docs/phase2-ai-triage.md`](docs/phase2-ai-triage.md) | The five pipeline stages, prompt design, and guardrails |
| [`docs/troubleshooting-journal.md`](docs/troubleshooting-journal.md) | Six resolved issues with verbatim error strings, root causes, and fixes |
| [`docs/screenshots/`](docs/screenshots/) | Sentinel console evidence: incident queue, correlated incident, entity mapping, resolution |
| [`detections/`](detections/) | The four detection rules as deployable KQL |
| [`src/`](src/) | Pipeline stages: [`auth`](src/auth.py), [`collect`](src/collect.py), [`enrich`](src/enrich.py), [`triage`](src/triage.py), [`writeback`](src/writeback.py) |
| [`prompts/triage_system.md`](prompts/triage_system.md) | The system prompt the triage stage runs against |
| [`evals/`](evals/) | [Ground truth](evals/ground_truth.json), [scorer](evals/score.py), [results](evals/results.md), and the [pre-fix triage fixture](evals/fixtures/incident-13-pre-fix-triage.json) |
...
The rule logic, entity mapping, and tuning notes are in [`docs/detections.md`](docs/detections.md), and the deployable KQL is in [`detections/`](detections/).
...
![The five-stage AI triage pipeline: collect, enrich, triage, writeback, and a separate evaluation stage that scores the triage output against ground truth](docs/pipeline.png)
...
Stage implementations are in [`src/`](src/), the triage prompt is in [`prompts/triage_system.md`](prompts/triage_system.md), and the full design writeup is in [`docs/phase2-ai-triage.md`](docs/phase2-ai-triage.md).
...
![The correlated incident in the Microsoft Sentinel console](docs/screenshots/correlation-incident.png)
...
The pre-fix triage output is preserved verbatim at [`evals/fixtures/incident-13-pre-fix-triage.json`](evals/fixtures/incident-13-pre-fix-triage.json), and the scoring methodology and full results are in [`evals/`](evals/).
...
- **Communicating to mixed audiences.** The same project is documented as a plain-language overview, a technical detection write-up, a troubleshooting journal with verbatim errors, and a rigorous evaluation report, each pitched at the reader who needs it.
